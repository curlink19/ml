...
from utils.bboxes_utils import iou_width_height, coco_to_yolo_tensors
import config
...
class Training_Dataset(Dataset):
    def __init__(
        ...
        assert bboxes_format in [
            "coco",
            "yolo",
        ], 'bboxes_format must be either "coco" or "yolo"'
        self.bs = bs
        ...
        try:
            self.annotations = pd.read_csv(
                os.path.join(root_directory, "labels", annot_file),
                header=None,
                index_col=0,
            ).sort_values(by=[0])
            self.annotations = self.annotations.head(
                (len(self.annotations) - 1)
            )  # just removes last line
        except FileNotFoundError:
            annotations = []
            for img_txt in os.listdir(
                os.path.join(self.root_directory, "labels", self.annot_folder)
            ):
            ...
            self.annotations.to_csv(
                os.path.join(self.root_directory, "labels", annot_file)
            )
    ...
    def __getitem__(self, idx):
        ...
        tg_height = (
            self.annotations.iloc[idx, 1] if self.rect_training else 640
        )
        ...
        img = np.array(
            Image.open(
                os.path.join(self.root_directory, self.fname, img_name)
            ).convert("RGB")
        )
        ...
        if self.bboxes_format == "coco":
            labels[
                :, -1
            ] -= 1  # 0-indexing the classes of coco labels (1-80 --> 0-79)
            labels = np.roll(labels, axis=1, shift=1)
            # normalized coordinates are scale invariant, hence after
            # resizing the img we don't resize labels
            labels[:, 1:] = coco_to_yolo_tensors(
                labels[:, 1:], w0=img.shape[1], h0=img.shape[0]
            )
        ...
        if self.transform:
            ...
            augmentations = self.transform(
                image=img, bboxes=np.roll(labels, axis=1, shift=4)
            )
        ...
        """if len(labels):
            plot_labes = xywhn2xyxy(labels[:, 1:]
        ...
        return (
            ...
            out_bboxes if self.ultralytics_loss else labels,
        )
    ...
    def adaptive_shape(self, annotations):
        ...
        path = os.path.join(
            self.root_directory,
            "labels",
            "adaptive_ann_{}_{}_br_{}.csv".format(
                name, self.len_ann, int(self.batch_range)
            ),
        )

        if os.path.isfile(path):
            print(
                "==> Loading cached annotations for rectangular"
                + f"training on {self.annot_folder}"
            )
            parsed_annot = pd.read_csv(path, index_col=0)
        else:
            print(
                "...Running adaptive_shape for 'rectangular training' on "
                + "training set..."
            )
            annotations["w_h_ratio"] = (
                annotations.iloc[:, 2] / annotations.iloc[:, 1]
            )
            annotations.sort_values(
                ["w_h_ratio"], ascending=True, inplace=True
            )

            for i in range(0, len(annotations), self.batch_range):
                size = [
                    annotations.iloc[i, 2],
                    annotations.iloc[i, 1],
                ]  # [width, height]
                max_dim = max(size)
                ...
                sz = (
                    random.randrange(
                        int(self.default_size * 0.9),
                        int(self.default_size * 1.1),
                    )
                    // 32
                    * 32
                )
                size[~max_idx] = (
                    (sz / size[max_idx]) * (size[~max_idx]) // 32
                ) * 32
                size[max_idx] = sz
                ...
                # sample annotation to avoid having
                # pseudo-equal images in the same batch
                annotations.iloc[i : i + bs, :] = annotations.iloc[
                    i : i + bs, :
...
class Validation_Dataset(Dataset):
    """COCO 2017 dataset constructed using the
    PyTorch built-in functionalities"""
    ...
    def __init__(
        ...
        """
        Parameters:
            train (bool): if true the os.path.join will lead to the train set,
             otherwise to the val set
            root_directory (path): path to the COCO2017 dataset
            transform: set of Albumentations transformations to be performed
            with A.Compose
        """
        assert bboxes_format in [
            "coco",
        ...
        self.anchors = torch.tensor(anchors).float().view(
            self.nl, -1, 2
        ) / torch.tensor(self.S).repeat(6, 1).T.reshape(3, 3, 2)
    ...
    def __getitem__(self, idx):
        ...
        tg_width = self.annotations.iloc[idx, 2] if self.rect_training else 640
        # print(f'image_name: {img_name}, idx: {idx}, tg_height: {tg_height},
        #  tg_width: {tg_width}')
        # img_name[:-4] to remove the .jpg or .png which are coco img formats
        label_path = os.path.join(
            os.path.join(
        ...
        # Below assumes 3 scale predictions (as paper) and
        #  same num of anchors per scale
        ...
        for idx, box in enumerate(bboxes):
            ...
            for anchor_idx in anchor_indices:
                ...
                # print(scale_idx)
                # anchor_on_scale searches the idx of the best
                #  anchor in a given scale
                ...
                # I guess [anchor_on_scale, i, j, 0] equals
                # to [anchor_on_scale][i][j][0]
                # check that the anchor hasn't been already taken
                # by another object (rare)
                anchor_taken = targets[scale_idx][anchor_on_scale, i, j, 4]
                # if not anchor_taken == if anchor_taken is still == 0
                # cause in the following
                # lines will be set to one
                # if not has_anchor[scale_idx] --> if this scale has
                # not been already taken
                # by another anchor which were ordered in descending
                # order by iou, hence
                ...
                if not anchor_taken and not has_anchor[scale_idx]:
                    ...
                    # setting p_o of the chosen cell = 1 since there
                    # is an object there
                    targets[scale_idx][anchor_on_scale, i, j, 4] = 1
                    # setting the values of the coordinates x, y
                    # i.e (6.5 - 6) = 0.5 --> x_coord is in the middle
    ...
    def adaptive_shape(self, annotations):
        ...
        if os.path.isfile(path):
            print(
                "==> Loading cached annotations for"
                + f"rectangular training on {self.annot_folder}"
            )
            parsed_annot = pd.read_csv(path, index_col=0)
        else:
            print(
                "...Running adaptive_shape for 'rectangular training' "
                + "on training set..."
            )
            annotations["w_h_ratio"] = (
                annotations.iloc[:, 2] / annotations.iloc[:, 1]
            ...
            for i in range(0, len(annotations), self.batch_range):
                ...
                # sample annotation to avoid having pseudo-equal images
                # in the same batch
                annotations.iloc[i : i + bs, :] = annotations.iloc[
                    i : i + bs, :
                ].sample(frac=1, axis=0)
